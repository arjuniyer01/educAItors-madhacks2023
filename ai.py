...
def generate_image_prompt(response):
    image_prompt = generate_response(
        "Generate a 50 word description for a non-violent image that aligns with the time period and the following text: "
        + response
    )
    return image_prompt
...
def generate_image(image_prompt):
    image_url = openai.Image.create(
        prompt=f"{image_prompt}, hd, oil painting", n=1, size="1024x1024"
    )["data"][0]["url"]
    return image_url
